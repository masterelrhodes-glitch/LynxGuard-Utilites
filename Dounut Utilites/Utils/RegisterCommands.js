...
async function RegisterCommands(client) {
  ...
  for (let i = 0; i < localCommands.length; i++) {
    ...
    try {
      if (!commandData)
        throw `No command.data found - Did you forget to save the file?`;
      ...
      Logs.error(
        `[REGISTER] Failed to register ${command.data.name}: ${error}`
      );
    }
  }
  ...
  if (devCommands.length > 0 && !config.DEV_GUILD_ID) {
    Logs.warn(
      `You have dev commands but no DISCORD.DEVELOPMENT_SERVER in config.json - These will not be registered!`
    );
  }
    ...
    req.on("response", (res) => {
      ...
      res.on("end", () => {
        try {
          if (!res.statusCode)
            throw new Error(
              "No status code returned - This is likely a network error"
            );
          if (res.statusCode < 200 || res.statusCode >= 300) {
            const response = JSON.parse(data.join(""));
            throw new Error(
              `[ Discord API : ${res.statusCode}] ${response.message}`
            );
          }
...
module.exports = async function DynamicRegister(client, force = false) {
  ...
  if (Date.now() - lastRegister < 1000 * 5) return;
