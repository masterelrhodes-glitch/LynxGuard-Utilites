...
const { CHECK_EVENT_NAMES } = require("../config.js");
...
module.exports = function (client, folderPath) {
  ...
  for (let i = 0; i < filePaths.length; i++) {
    ...
    try {
      ...
      if (
        CHECK_EVENT_NAMES &&
        !IGNORED_EVENTS.includes(data.name) &&
        !Object.values(Events).includes(data.name)
      ) {
        Logs.warn(
          `Possibly invalid event name "${data.name}" - Unless it is a custom event this will never be called!`
        );
      }

      if (typeof data.execute !== "function")
        throw `Event is missing an execute function!`;
